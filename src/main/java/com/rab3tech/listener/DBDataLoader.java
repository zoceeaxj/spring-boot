 ...
 * servletContext); // ProfileDao //
 * profileDao=applicationContext.getBean(ProfileDaoImpl.class); ServletContext
 * servletContext=arg0.getServletContext(); ApplicationContext
 * applicationContext=WebApplicationContextUtils.getWebApplicationContext(
 * servletContext);
 * 
 * try { File file = ResourceUtils.getFile("classpath:db/script.sql"); String
 * content = FileUtils.readFileToString(file); Connection conn = null; try {
 * DataSource dataSource=(DataSource)applicationContext.getBean("pdataSource");
 * conn = dataSource.getConnection();
 * System.out.println("********************Connection****************"+conn);
 * boolean execute = false; try { PreparedStatement preparedStatement =
 * conn.prepareStatement("select username from user_login_tbl"); ResultSet rs =
 * preparedStatement.executeQuery(); if (!rs.next()) { execute = true; } } catch
 * (Exception e) { System.out.println(e.getMessage()); execute = true; }
 * 
 * if (execute) { conn.setAutoCommit(false); ScriptUtils.executeSqlScript(conn,
 * new ByteArrayResource(content.getBytes())); System.out.
 * println("~~~~~~~~~~~~~~~~~~~~Script is executed successfully for the database"
 * ); conn.commit();// }
 * 
 * } catch (Exception e) { if (conn != null) { try { conn.rollback();
 * conn.close(); } catch (SQLException e1) { // TODO Auto-generated catch block
